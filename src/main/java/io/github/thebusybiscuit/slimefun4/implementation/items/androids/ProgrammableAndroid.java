...
public class ProgrammableAndroid extends SlimefunItem implements InventoryBlock, RecipeDisplayItem {
    ...
    public void openScript(Player p, Block b, String sourceCode) {
        ...
        menu.addItem(0, new CustomItem(Instruction.START.getItem(), SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions.START"), "", "&7\u21E8 &e左鍵 &7返回機器人控制台"));
        ...
        for (int i = 1; i < script.length; i++) {
            ...
            if (i == script.length - 1) {
                ...
                menu.addItem(slot, new CustomItem(Instruction.REPEAT.getItem(), SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions.REPEAT"), "", "&7\u21E8 &e左鍵 &7返回機器人控制台"));
    ...
    protected void openScriptDownloader(Player p, Block b, int page) {
        ...
        menu.addItem(48, new CustomItem(HeadTexture.SCRIPT_UP.getAsItemStack(), "&e上傳腳本", "", "&6單擊 &7將你正在使用的腳本", "&7上傳到服務器"));
    ...
    public void openScriptEditor(Player p, Block b) {
        ...
        menu.addItem(1, new CustomItem(HeadTexture.SCRIPT_FORWARD.getAsItemStack(), "&2> 編輯腳本", "", "&aEdits your current Script"));
        menu.addMenuClickHandler(1, (pl, slot, item, action) -> {
            openScript(pl, b, getScript(b.getLocation()));
            return false;
        });

        menu.addItem(3, new CustomItem(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&4> 創建新腳本", "", "&cDeletes your current Script", "&cand creates a blank one"));
    ...
    protected void editInstruction(Player p, Block b, String[] script, int index) {
        ...
        menu.setEmptySlotsClickable(false);
        menu.addItem(9, new CustomItem(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&f待命"), (pl, slot, item, action) -> {
            String code = deleteInstruction(script, index);
            setScript(b.getLocation(), code);
            openScript(p, b, code);
            return false;
